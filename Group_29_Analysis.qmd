...
The AIC of `stepAIC_model` is the smallest, indicating the best model fit. While it includes more variables, it does not suffer from overfitting. Additionally, `stepAIC_model` achieves the highest AUC of 0.881, demonstrating the strongest ability to classify high-income (\>50K) and low-income (\<=50K) groups.
